class_name LayeredRockBatchLayer
extends Node2D
...
const RESULT_KEY: StringName = &"rock_atlas_bucket_buffers"
...
func configure_catalog(catalog: AssetCatalog) -> void:
	if catalog == null or not catalog.is_ready():
		push_error("LayeredRockBatchLayer: a boot-prepared asset catalog is required")
		return
	_catalog = catalog
...
func begin_apply(native_result: Dictionary) -> bool:
	if _catalog == null or not _catalog.is_ready():
		push_error("LayeredRockBatchLayer: configure_catalog() must run before chunk publish")
		return false
...
## See LayeredTreeBatchLayer.reserve_pool_slots(). This moves the first
## MultiMesh/CanvasItem creation to the bounded presentation-envelope pool.
...
## a fixed 64-entry lookup table and are skipped in the same slice so sparse
## object packets do not acquire dozens of no-op frames.
...
func shrink_pool_next_slot() -> bool:
	...
	for key: StringName in [&"albedo", &"snow", &"shadow"]:
...
func get_debug_state() -> Dictionary:
	...
	return {
		...
		"canvas_item_count": 1 + _slots.size() * 3 \
...
func _validated_buffers(value: Variant) -> Array[PackedFloat32Array]:
	...
	if not value is Array:
		push_error("LayeredRockBatchLayer: missing %s" % RESULT_KEY)
		return result
	var source: Array = value as Array
	if source.size() != STRIPE_COUNT:
		push_error("LayeredRockBatchLayer: %s must contain %d stripes" % [RESULT_KEY, STRIPE_COUNT])
		return result
	for stripe_index: int in range(STRIPE_COUNT):
		if not source[stripe_index] is PackedFloat32Array:
			push_error("LayeredRockBatchLayer: stripe %d has an invalid buffer" % stripe_index)
			return []
		var buffer: PackedFloat32Array = source[stripe_index] as PackedFloat32Array
		if buffer.size() % MULTIMESH_BUFFER_STRIDE != 0:
			push_error("LayeredRockBatchLayer: stripe %d violates raw MultiMesh stride" % stripe_index)
			return []
...
func _create_slot(slot_index: int) -> Dictionary:
	...
	var albedo: MultiMeshInstance2D = _make_layer(
		"RockBatch%dAlbedo" % slot_index,
		AssetCatalog.ROCK_ALBEDO_ATLAS,
		_catalog.get_rock_albedo_material(),
		visual_multimesh,
	)
	var snow: MultiMeshInstance2D = _make_layer(
		"RockBatch%dSnow" % slot_index,
		AssetCatalog.ROCK_SNOW_OVERLAY_ATLAS,
		_catalog.get_rock_snow_material(),
		visual_multimesh,
	)
	var shadow: MultiMeshInstance2D = _make_layer(
		"RockBatch%dShadow" % slot_index,
		AssetCatalog.ROCK_SHADOW_ATLAS,
		_catalog.get_rock_shadow_material(),
		shadow_multimesh,
	)
	var slot: Dictionary = {
		"stripe": -1,
...
func _ensure_depth_ladder() -> DepthLadderBandRoot:
	...
	_depth_ladder.name = "RockDepthLadder"
