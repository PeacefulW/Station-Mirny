## Bush family batch layer.
##
## Bushes are baked on the tree contract and therefore ship the same seven
## channels, so this layer is the tree batch layer bound to the bush atlas set:
## same stripe pooling, same depth ladder, same sun-shadow split. The families
## stay separate because their atlas layouts, sizes and scatter rules differ,
## and because a bush must never acquire the tree trunk collider.
class_name LayeredBushBatchLayer
extends Node2D
...
const RESULT_KEY: StringName = &"bush_atlas_bucket_buffers"
...
func configure_catalog(catalog: AssetCatalog) -> void:
	if catalog == null or not catalog.is_ready():
		push_error("LayeredBushBatchLayer: a boot-prepared asset catalog is required")
		return
	_catalog = catalog
...
func begin_apply(native_result: Dictionary) -> bool:
	if _catalog == null or not _catalog.is_ready():
		push_error("LayeredBushBatchLayer: configure_catalog() must run before chunk publish")
		return false
...
func apply_next_batch(max_stripes: int) -> bool:
	...
	if not _has_pending_apply:
		return false
	# Cold capacity reservation is an explicit phase. Front-load exactly one
	# missing slot and return before touching a raw MultiMesh buffer; the caller
	# may then schedule another allocation-only callback from measured lookahead.
...
## Allocates exactly one pooled stripe graph without advancing the packet
## cursor or uploading any worker buffer. Returns true only when capacity grew.
...
## Releases at most max_slots resident slot groups. One bush slot owns two raw
...
func _validated_buffers(value: Variant) -> Array[PackedFloat32Array]:
	...
	if not value is Array:
		push_error("LayeredBushBatchLayer: missing %s" % RESULT_KEY)
		return result
	var source: Array = value as Array
	if source.size() != STRIPE_COUNT:
		push_error("LayeredBushBatchLayer: %s must contain %d stripes" % [RESULT_KEY, STRIPE_COUNT])
		return result
	for stripe_index: int in range(STRIPE_COUNT):
		if not source[stripe_index] is PackedFloat32Array:
			push_error("LayeredBushBatchLayer: stripe %d has an invalid buffer" % stripe_index)
			return []
		var buffer: PackedFloat32Array = source[stripe_index] as PackedFloat32Array
		if buffer.size() % MULTIMESH_BUFFER_STRIDE != 0:
			push_error("LayeredBushBatchLayer: stripe %d violates raw MultiMesh stride" % stripe_index)
			return []
...
func _create_slot(slot_index: int) -> Dictionary:
	...
	var shadow_multimesh: MultiMesh = _make_multimesh(_catalog.get_bush_shadow_mesh())
	var trunk: MultiMeshInstance2D = _make_layer(
		"BushBatch%dTrunk" % slot_index,
		AssetCatalog.BUSH_TRUNK_ATLAS,
		_catalog.get_bush_trunk_material(),
		visual_multimesh,
	)
	var foliage: MultiMeshInstance2D = _make_layer(
		"BushBatch%dFoliage" % slot_index,
		AssetCatalog.BUSH_FOLIAGE_ATLAS,
		_catalog.get_bush_foliage_material(),
		visual_multimesh,
	)
	var snow: MultiMeshInstance2D = _make_layer(
		"BushBatch%dSnow" % slot_index,
		AssetCatalog.BUSH_SNOW_OVERLAY_ATLAS,
		_catalog.get_bush_snow_material(),
		visual_multimesh,
	)
	var shadow: MultiMeshInstance2D = _make_layer(
		"BushBatch%dShadow" % slot_index,
		AssetCatalog.BUSH_SHADOW_ATLAS,
		_catalog.get_bush_shadow_material(),
		shadow_multimesh,
	)
	var slot: Dictionary = {
		"stripe": -1,
	...
	WorldPerfProbe.end(
		"WorldObjectPacketLayer.bush.slot_allocate",
...
func _sync_slot(slot: Dictionary, stripe_index: int, buffer: PackedFloat32Array) -> void:
	...
	WorldPerfProbe.end(
		"WorldObjectPacketLayer.bush.raw_visual_upload",
	...
	WorldPerfProbe.end(
		"WorldObjectPacketLayer.bush.raw_shadow_upload",
	...
	# with the bush feet would change their visual contract.
	(slot.get("shadow") as MultiMeshInstance2D).z_index = WorldRuntimeConstants.Z_CAST_SHADOW
	WorldPerfProbe.end("WorldObjectPacketLayer.bush.ladder_register", ladder_started_usec)
...
func _ensure_depth_ladder() -> DepthLadderBandRoot:
	...
	_depth_ladder.name = "BushDepthLadder"
