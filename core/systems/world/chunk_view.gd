...
func begin_apply(terrain_ids: PackedInt32Array) -> void:
...
func apply_next_batch(batch_size: int) -> bool:
	...
	for index: int in range(_apply_index, end_index):
		...
		_apply_cell(local_coord, terrain_id)
...
func apply_runtime_tile(local_coord: Vector2i, terrain_id: int) -> void:
	_ensure_tile_layer()
	_apply_cell(local_coord, terrain_id)
...
func _apply_cell(local_coord: Vector2i, terrain_id: int) -> void:
	...
	_tile_layer.set_cell(
		...
		WorldTileSetFactory.get_atlas_coords(terrain_id)
	)
