class_name WorldLivingFloraScatterLayer
extends Node2D

const WorldRuntimeConstants = preload("res://core/systems/world/world_runtime_constants.gd")
const WorldDecorBatchLayer = preload("res://core/systems/world/world_decor_batch_layer.gd")

const FRAME_COLUMNS: int = 16
const FRAME_ROWS: int = 4
const FRAMES_PER_VIEW: int = 16
const FRAME_COUNT: int = FRAME_COLUMNS * FRAME_ROWS
const SOUTH_FACING_VIEW_INDEX: int = 0
const SCATTER_GRID_SIDE: int = 10
const PRIMARY_DENSITY: float = 0.42
const MAX_PLANTS_PER_CHUNK: int = 18
const EDGE_PADDING_PX: float = 24.0
const MIN_SIZE_PX: float = 84.0
const MAX_SIZE_PX: float = 112.0
const GRASS_PATCH_CELL_PX: float = 1850.0
const GRASS_PATCH_DENSITY: float = 0.54
const GRASS_PATCH_THRESHOLD: float = 0.50
const GRASS_EDGE_CLEARANCE_PX: float = 22.0
const TERRAIN_THRESHOLD_LOW: float = 0.40
const TERRAIN_THRESHOLD_HIGH: float = 0.58
const ANIMATION_FPS: float = 7.0
const SHADOW_WIDTH_SCALE: float = 0.42
const SHADOW_HEIGHT_SCALE: float = 0.13
const SHADOW_CENTER_Y_SCALE: float = 0.32
const SHADOW_MIN_WIDTH_PX: float = 10.0
const SHADOW_MIN_HEIGHT_PX: float = 4.0
...
# native chunk packet path when generated object placement is promoted.
...
func _rebuild_batch() -> void:
	...
	batch_layer.set_animation(FRAMES_PER_VIEW, ANIMATION_FPS)
...
func _append_living_flora(
	...
	if not _is_position_valid_for_living_flora(position):
		return
	var base_frame_index: int = SOUTH_FACING_VIEW_INDEX * FRAMES_PER_VIEW
	var size_px: float = lerpf(MIN_SIZE_PX, MAX_SIZE_PX, _unit_float(_hash4(salt, 101, 103, 107)))
	var tint_factor: float = lerpf(0.86, 1.04, _unit_float(_hash4(salt, 109, 113, 127)))
	var phase: float = _unit_float(_hash4(salt, 131, 137, 139))
...
func _is_position_valid_for_living_flora(position: Vector2) -> bool:
	var chunk_size_px: float = float(WorldRuntimeConstants.CHUNK_SIZE * WorldRuntimeConstants.TILE_SIZE_PX)
	...
	return _grass_patch_score(world_pos) >= GRASS_PATCH_THRESHOLD

func _grass_patch_score(world_px: Vector2) -> float:
	...
	var clearance_texels: float = maxf(GRASS_EDGE_CLEARANCE_PX / maxf(_mask_step_px, 0.001), 1.0)
	...
	if edge_safe <= 0.01:
		return 0.0
	var patch: float = _organic_region_mask(world_px)
	return patch * edge_safe * ground
...
func _organic_region_mask(world_px: Vector2) -> float:
	var p: Vector2 = world_px / maxf(GRASS_PATCH_CELL_PX, 1.0)
	...
	var threshold: float = lerpf(0.76, 0.43, clampf(GRASS_PATCH_DENSITY, 0.0, 1.0))
...
func _ensure_batch_layer() -> WorldDecorBatchLayer:
	...
	_batch_layer.name = "LivingFloraDecorBatchLayer"
