class_name PlainsTreePlacementSettings
extends Resource
...
const MAX_PER_CHUNK_MAX: int = 128
const SIZE_MIN: float = 1.0
const SIZE_MAX: float = 254.0

@export_group("Identity")
@export var id: StringName = &"core:plains_trees"
@export var object_family: StringName = &"tree"
@export var biome_id: StringName = &"core:plains"
@export var placement_mask: StringName = &"grass_field"

@export_group("Placement")
@export_range(0.0, 1.0, 0.01) var density: float = 0.62
@export_range(1, 16) var scatter_grid_side: int = 6
@export_range(0, 128) var max_per_chunk: int = 0
@export_range(0.0, 256.0) var edge_padding_px: float = 40.0
@export_range(0.0, 512.0) var min_distance_px: float = 88.0
@export_range(0.0, 1.0, 0.01) var grass_density_min: float = 0.40

@export_group("Size Tiers")
@export_range(1.0, 254.0) var visual_size_min_px: float = 150.0
@export_range(1.0, 254.0) var visual_size_max_px: float = 244.0
@export_range(0.0, 1.0, 0.01) var small_chance: float = 0.20
@export_range(1.0, 254.0) var small_visual_size_px: float = 120.0
@export_range(0.0, 1.0, 0.01) var hero_chance: float = 0.10
@export_range(1.0, 254.0) var hero_visual_size_px: float = 252.0
...
func to_save_dict() -> Dictionary:
	return {
		...
		"small_visual_size_px": small_visual_size_px,
		"hero_chance": hero_chance,
		"hero_visual_size_px": hero_visual_size_px,
...
func write_to_settings_packed(settings_packed: PackedFloat32Array) -> PackedFloat32Array:
	var settings: PlainsTreePlacementSettings = from_save_dict(to_save_dict())
	var packed: PackedFloat32Array = settings_packed.duplicate()
	packed.resize(WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_FIELD_COUNT)
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_DENSITY] = settings.density
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_SCATTER_GRID_SIDE] = float(settings.scatter_grid_side)
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_EDGE_PADDING_PX] = settings.edge_padding_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_MIN_DISTANCE_PX] = settings.min_distance_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_MAX_PER_CHUNK] = float(settings.max_per_chunk)
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_MIN_SIZE_PX] = settings.visual_size_min_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_MAX_SIZE_PX] = settings.visual_size_max_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_SMALL_CHANCE] = settings.small_chance
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_SMALL_SIZE_PX] = settings.small_visual_size_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_HERO_CHANCE] = settings.hero_chance
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_HERO_SIZE_PX] = settings.hero_visual_size_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_GRASS_DENSITY_MIN] = settings.grass_density_min
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_GRASS_FIELD_SCALE_PX] = settings.grass_field_scale_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_GRASS_COVERAGE] = settings.grass_coverage
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_ROCK_FIELD_SCALE_PX] = settings.rock_field_scale_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_ROCK_COVERAGE] = settings.rock_coverage
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_MACRO_MASS_SCALE_PX] = settings.macro_mass_scale_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_MACRO_MASS_STRENGTH] = settings.macro_mass_strength
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_PATH_SCALE_PX] = settings.path_scale_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_PATH_WIDTH] = settings.path_width
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_PATH_WARP_PX] = settings.path_warp_px
	packed[WorldRuntimeConstants.SETTINGS_PACKED_LAYOUT_TREE_PATH_STRENGTH] = settings.path_strength
	return packed


static func from_save_dict(data: Dictionary) -> PlainsTreePlacementSettings:
	var settings: PlainsTreePlacementSettings = hard_coded_defaults()
	...
	settings.small_chance = clampf(_read_float(data, "small_chance", settings.small_chance), 0.0, 1.0)
	settings.small_visual_size_px = clampf(
		_read_float(data, "small_visual_size_px", settings.small_visual_size_px),
		SIZE_MIN,
		SIZE_MAX,
	)
	settings.hero_chance = clampf(_read_float(data, "hero_chance", settings.hero_chance), 0.0, 1.0)
	settings.hero_visual_size_px = clampf(
		_read_float(data, "hero_visual_size_px", settings.hero_visual_size_px),
		SIZE_MIN,
		SIZE_MAX,
	)
...
static func hard_coded_defaults() -> PlainsTreePlacementSettings:
	return new()
