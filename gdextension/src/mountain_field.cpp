...
uint8_t Evaluator::resolve_mountain_flags(
	...
	if (is_wall) {
		...
		if (world_version_ < k_hierarchical_world_version && is_anchor_tile(p_world_x, p_world_y, p_center_mountain_id)) {
...
HierarchicalMacroSolve solve_hierarchical_macro(
	...
	if (!uses_hierarchical_labeling(p_world_version)) {
		return HierarchicalMacroSolve{};
	}
