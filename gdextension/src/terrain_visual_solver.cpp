...
std::vector<EmptyTileRect> collect_empty_tile_rects(
	const PackedByteArray &p_solid_mask,
	int64_t p_width_tiles,
	int64_t p_height_tiles,
	int32_t p_tile_size_px
) {
	std::vector<EmptyTileRect> empty_rects;
	empty_rects.reserve(static_cast<size_t>((p_width_tiles + 2) * (p_height_tiles + 2)));
	...
	for (int64_t tile_y = -1; tile_y <= p_height_tiles; ++tile_y) {
		for (int64_t tile_x = -1; tile_x <= p_width_tiles; ++tile_x) {
			...
			const double min_x = static_cast<double>(tile_x * p_tile_size_px);
			const double min_y = static_cast<double>(tile_y * p_tile_size_px);
			empty_rects.push_back({
				min_x,
				min_y,
				min_x + static_cast<double>(p_tile_size_px),
				min_y + static_cast<double>(p_tile_size_px),
			});
		}
	}
...
NearestBoundary find_nearest_empty_boundary(
	const std::vector<EmptyTileRect> &p_empty_rects,
	double p_x,
	double p_y
) {
	NearestBoundary nearest;

	for (const EmptyTileRect &rect : p_empty_rects) {
		const double closest_x = std::clamp(p_x, rect.min_x, rect.max_x);
		const double closest_y = std::clamp(p_y, rect.min_y, rect.max_y);
		...
		if (distance_sq >= nearest.distance_px * nearest.distance_px) {
			continue;
		}
...
void write_q16(PackedByteArray &r_bytes, int32_t p_pixel_index, double p_value) {
	...
	r_bytes.set(offset, q16 & 0xff);
	r_bytes.set(offset + 1, (q16 >> 8) & 0xff);
}
...
void write_normal_rgba8(
	...
	r_bytes.set(offset, static_cast<uint8_t>(std::round((nx * 0.5 + 0.5) * 255.0)));
	r_bytes.set(offset + 1, static_cast<uint8_t>(std::round((ny * 0.5 + 0.5) * 255.0)));
	r_bytes.set(offset + 2, static_cast<uint8_t>(std::round((nz * 0.5 + 0.5) * 255.0)));
	r_bytes.set(offset + 3, 255);
}
...
Dictionary TerrainVisualSolver::build_editor_preview_packet(
	...
	ERR_FAIL_COND_V_MSG(
		...
		packet,
		"TerrainVisualSolver.build_editor_preview_packet received a negative mask dimension."
	);
	...
	ERR_FAIL_COND_V_MSG(
		...
		packet,
		"TerrainVisualSolver.build_editor_preview_packet received a mask size that does not match width * height."
	);

	const RecipePayload recipe = unpack_recipe_payload(p_recipe_payload);
	const int32_t pixel_width = checked_pixel_size(p_width_tiles, recipe.tile_size_px);
	const int32_t pixel_height = checked_pixel_size(p_height_tiles, recipe.tile_size_px);
	...
	ERR_FAIL_COND_V_MSG(
		...
		packet,
		"TerrainVisualSolver.build_editor_preview_packet received a mask that is too large for one packet."
	);

	const int32_t pixel_count = static_cast<int32_t>(pixel_count_64);
	const std::vector<EmptyTileRect> empty_rects = collect_empty_tile_rects(
		p_solid_mask,
		p_width_tiles,
		p_height_tiles,
		recipe.tile_size_px
	...
	for (int32_t y = 0; y < pixel_height; ++y) {
		for (int32_t x = 0; x < pixel_width; ++x) {
			...
			const int64_t tile_x = x / recipe.tile_size_px;
			const int64_t tile_y = y / recipe.tile_size_px;
			...
			if (solid) {
				++solid_pixels;
				boundary = find_nearest_empty_boundary(empty_rects, sample_x, sample_y);
			...
			zone_ids.set(pixel_index, zone);
			coverage_top.set(pixel_index, zone == ZONE_TOP ? 255 : 0);
			coverage_edge.set(pixel_index, zone == ZONE_EDGE ? 255 : 0);
			coverage_face.set(pixel_index, zone == ZONE_FACE ? 255 : 0);
			coverage_back.set(pixel_index, zone == ZONE_BACK ? 255 : 0);
			write_q16(height_q16, pixel_index, height);
			write_material_uv(
				material_u_q16,
				material_v_q16,
				pixel_index,
	...
	packet["world_origin_tile"] = p_preview_origin_tile;
	packet["chunk_coord"] = Vector2i(0, 0);
	packet["dirty_rect_tiles"] = Rect2i(
		Vector2i(0, 0),
