...
def configure_lighting(radius: float) -> None:
    ...
    add_shadowless_light("neutral_top_front_key_no_shadow", (0.0, -3.4 * scale, 5.8 * scale), 460.0, 6.0 * scale)
    add_shadowless_light("neutral_top_fill_no_shadow", (0.0, 1.8 * scale, 5.8 * scale), 230.0, 8.0 * scale)
    add_shadowless_light("neutral_left_fill_no_shadow", (-4.0 * scale, 0.8 * scale, 4.0 * scale), 120.0, 7.0 * scale)
...
def main() -> None:
    ...
    if len(args) != 3 and len(args) != 4:
        raise SystemExit(
            "Usage: blender --background --python render_shadowless_single_sprite.py -- "
            "<input.glb> <output.png> <frame_size> [frame_padding]"
        )
    input_glb = Path(args[0])
    output_png = Path(args[1])
    frame_size = int(args[2])
    frame_padding = float(args[3]) if len(args) >= 4 else 1.14
    ...
    mins, maxs = mesh_bounds(meshes)
    print("Shadowless single sprite render complete")
