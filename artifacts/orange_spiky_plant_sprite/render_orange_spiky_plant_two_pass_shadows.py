...
def add_sun(radius: float) -> bpy.types.Object:
    ...
    sun.location = (0.0, GAME_TOP_FRONT_CAMERA_Y_SIGN * 0.46 * scale, 6.2 * scale)
...
def configure_lighting(radius: float) -> list[bpy.types.Object]:
    ...
    lights = [
        ...
        add_shadowless_fill("soft_left_fill_no_shadow", (-3.6 * scale, 1.8 * scale, 4.3 * scale), 30.0, 7.0 * scale),
    ]
    return lights
...
def main() -> None:
    ...
    if len(args) != 5:
        raise SystemExit(
            "Usage: blender --background --python render_orange_spiky_plant_two_pass_shadows.py -- "
    ...
    print("Orange spiky plant two-pass render complete")
