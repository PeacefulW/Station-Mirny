...
def parent_roots_to_turntable(imported: list[bpy.types.Object]) -> bpy.types.Object:
    imported_set = set(imported)
    roots = [obj for obj in imported if obj.parent not in imported_set]
    root = bpy.data.objects.new("orange_spiky_plant_turntable_root", None)
...
def add_shadowless_fill(name: str, location: tuple[float, float, float], power: float, size: float) -> None:
    ...
    look_at(obj, Vector((0.0, 0.0, 0.28)))


def configure_lighting(radius: float) -> None:
    scale = max(radius, 1.0)
    add_sun(radius)
    add_shadowless_fill("soft_front_fill_no_shadow", (0.0, -4.2 * scale, 5.0 * scale), 70.0, 6.5 * scale)
    add_shadowless_fill("soft_left_fill_no_shadow", (-3.6 * scale, 1.8 * scale, 4.3 * scale), 32.0, 7.0 * scale)
...
def add_shadow_catcher(radius: float) -> bpy.types.Object:
    ...
    plane.name = "transparent_shadow_catcher_ground"
    try:
    ...
    material = bpy.data.materials.new("transparent_shadow_catcher_material")
    material.diffuse_color = (0.39, 0.34, 0.27, 1.0)
...
def render_rotations(turntable_root: bpy.types.Object, output_dir: Path, rotations: int, asset_name: str) -> None:
    ...
    for index in range(rotations):
        ...
        turntable_root.rotation_euler.z = math.tau * index / rotations
        bpy.context.view_layer.update()
        scene.render.filepath = str(output_dir / f"{asset_name}_rot_{index:02d}_{degrees:03d}deg_raw.png")
...
def main() -> None:
    ...
    if len(args) != 5:
        raise SystemExit(
            "Usage: blender --background --python render_orange_spiky_plant_sprites.py -- "
    ...
    print("Orange spiky plant sprite render complete")
    ...
    print("shadow=cycles transparent shadow catcher, top sun with slight front tilt")
