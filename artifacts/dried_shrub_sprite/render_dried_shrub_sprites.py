...
def parent_roots_to_turntable(imported: list[bpy.types.Object]) -> bpy.types.Object:
    imported_set = set(imported)
    roots = [obj for obj in imported if obj.parent not in imported_set]
    root = bpy.data.objects.new("dried_shrub_turntable_root", None)
...
def configure_shadowless_materials(meshes: list[bpy.types.Object]) -> None:
    for obj in meshes:
        ...
        for slot in obj.material_slots:
            ...
            if material is None:
                continue
            material.use_nodes = material.use_nodes
...
def add_shadowless_light(name: str, location: tuple[float, float, float], power: float, size: float) -> None:
    ...
    look_at(obj, Vector((0.0, 0.0, 0.4)))


def configure_lighting(radius: float) -> None:
    scale = max(radius, 1.0)
    add_shadowless_light("soft_front_top_no_shadow", (0.0, -4.0 * scale, 5.5 * scale), 420.0, 5.0 * scale)
    add_shadowless_light("soft_fill_no_shadow", (-3.5 * scale, 2.5 * scale, 4.0 * scale), 130.0, 6.0 * scale)
    add_shadowless_light("soft_rim_no_shadow", (3.5 * scale, 3.0 * scale, 4.5 * scale), 80.0, 6.0 * scale)
...
def configure_camera(meshes: list[bpy.types.Object], turntable_root: bpy.types.Object, rotations: int) -> bpy.types.Object:
    ...
    # The GLB has sparse/transparent extremities, so a tighter orthographic
    # frame gives a better game sprite while keeping all visible alpha inside.
    camera_data.ortho_scale = max_extent * 0.90
    return camera
...
def render_rotations(turntable_root: bpy.types.Object, output_dir: Path, rotations: int) -> None:
    ...
    for index in range(rotations):
        ...
        turntable_root.rotation_euler.z = math.tau * index / rotations
        bpy.context.view_layer.update()
        scene.render.filepath = str(output_dir / f"dried_shrub_rot_{index:02d}_{degrees:03d}deg.png")
...
def main() -> None:
    ...
    if len(args) != 4:
        raise SystemExit("Usage: blender --background --python render_dried_shrub_sprites.py -- <input.glb> <output_dir> <rotations> <frame_size>")
    ...
    mins, maxs = mesh_bounds(meshes)
    print("Dried shrub sprite render complete")
