...
def main() -> None:
    if len(sys.argv) != 5:
        raise SystemExit("Usage: python make_spritesheet.py <frames_dir> <sheet_png> <metadata_json> <columns>")
    ...
    frame_paths = sorted(frames_dir.glob("dried_shrub_rot_*_*.png"))
    if not frame_paths:
    ...
    metadata = {
        "source": "M:/Downloads/dried shrub 3d model.glb",
