...
def render_one(input_glb: Path, output_png: Path, frame_size: int, frame_padding: float) -> None:
    clear_scene()
    ...
    if not meshes:
        raise RuntimeError(f"No mesh objects were imported from {input_glb}.")
    ...
    print("Self-shadowed small rock sprite render complete")
...
def main() -> None:
    ...
    if len(args) != 4 and len(args) != 5:
        raise SystemExit(
            "Usage: blender --background --python render_self_shadowed_small_rock_atlas.py -- "
            "<input_dir> <output_dir> <frame_size> <frame_padding> [name_prefix]"
        )
    input_dir = Path(args[0])
    output_dir = Path(args[1])
    frame_size = int(args[2])
    frame_padding = float(args[3])
    name_prefix = args[4] if len(args) >= 5 else "plains_grass_edge_small_rock"
    if not input_dir.exists():
        raise FileNotFoundError(input_dir)
    glbs = sorted(input_dir.glob("*.glb"), key=lambda path: path.name.lower())
    if not glbs:
        raise FileNotFoundError(f"No GLB files found in {input_dir}.")
    for index, input_glb in enumerate(glbs):
        output_png = output_dir / f"{name_prefix}_{index:02d}.png"
        render_one(input_glb, output_png, frame_size, frame_padding)
    print(f"rendered_count={len(glbs)}")
