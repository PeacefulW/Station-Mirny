...
# Процедурные деревья на РЕАЛЬНОЙ равнине + тени-силуэты, привязанные к солнцу.
# Холст запекания — по фактическому bounding box дерева (без обрезки кроны/корней),
# плюс лист проверки текстур (proc_bake_check.png). Оценочная проба. Windowed:
#   Godot_v4.7-stable_win64_console.exe --path . -s artifacts/tree_proc_sketch/proc_tree_field_probe.gd
...
const OUTPUT_DIR: String = "res://artifacts/tree_proc_sketch"
const TREE_WIND_SHADER: String = "res://artifacts/tree_proc_sketch/tree_wind.gdshader"
const CAPTURE_HOUR: float = 9.0
...
func _run() -> void:
	if DisplayServer.get_name() == "headless":
		push_error("proc_tree_field_probe: must run windowed")
		quit(1)
		return

	# Солнце по времени суток (каноничная модель TimeManager).
	var sun_angle: float = _sun_angle_for_hour(CAPTURE_HOUR)
	var light: Vector2 = Vector2(cos(sun_angle), sin(sun_angle))
	_shadow_dir = (-light).normalized()
	_shadow_foreshorten = clampf(0.5 * _shadow_len_for_hour(CAPTURE_HOUR), 0.4, 1.3)
	_bake_light_dir = Vector2(clampf(light.x, -0.85, 0.85), -0.72).normalized()
	print("proc_tree_field_probe: sun hour=%.1f shadow_dir=%s foreshorten=%.2f" % [CAPTURE_HOUR, str(_shadow_dir), _shadow_foreshorten])
	...
	if _tex.is_empty():
		print("proc_tree_field_probe: FAIL no tree textures baked")
		quit(1)
		return
	print("proc_tree_field_probe: baked %d tree textures" % _tex.size())
	_save_bake_check()
	...
	DirAccess.open("res://").make_dir_recursive("artifacts/tree_proc_sketch")

	var home: Vector2 = player.global_position
	await _settle()

	# Деревья — фиксированный слой НАД травой: трава целиком дерево не перекрывает
	# и эффект не зависит от игрока. «Трава у комля» запечена в текстуру.
	...
	print("proc_tree_field_probe: planted %d trees" % planted)
	if planted <= 0:
		print("proc_tree_field_probe: FAIL no plains tiles to plant on")
		quit(1)
		return
	...
	for zoom: float in [1.0, 0.7, 0.45]:
		camera.zoom = Vector2(zoom, zoom)
		camera.set("_target_zoom", zoom)
		camera.force_update_scroll()
		await _wait_frames(10)
		var img: Image = await _capture()
		img.save_png("%s/proc_field_zoom_%03d.png" % [OUTPUT_DIR, int(zoom * 100.0)])
		print("proc_tree_field_probe: saved proc_field_zoom_%03d.png" % int(zoom * 100.0))
	...
	var no_shadow: Image = await _capture()
	_shadow_root.visible = true
	await _wait_frames(4)
	var with_shadow: Image = await _capture()
	_save_panel([no_shadow, with_shadow], "%s/proc_shadow_panel.png" % OUTPUT_DIR)

	# проба качания
	var wind: Node = root.get_node_or_null("WindRuntime")
	if wind != null and time_manager != null:
		camera.zoom = Vector2(0.95, 0.95)
		camera.set("_target_zoom", 0.95)
		camera.force_update_scroll()
		wind.call("set_debug_direction_override_deg", 25.0)
		wind.call("set_debug_strength_override", 0.78)
		time_manager.call("set_paused", false)
		await _wait_frames(8)
		var sway_a: Image = await _capture()
		await _wait_frames(28)
		var sway_b: Image = await _capture()
		time_manager.call("set_paused", true)
		wind.call("clear_debug_wind_override")
		_save_panel([sway_a, sway_b], "%s/proc_wind_sway_panel.png" % OUTPUT_DIR)
		print("proc_tree_field_probe: wind sway frame diff=%.5f" % _mean_abs_diff(sway_a, sway_b))

	camera.zoom = Vector2(1.7, 1.7)
	camera.set("_target_zoom", 1.7)
	camera.force_update_scroll()
	await _wait_frames(12)
	var closeup: Image = await _capture()
	closeup.save_png("%s/proc_field_closeup.png" % OUTPUT_DIR)
	var crop := Vector2i(720, 540)
	var origin := Vector2i((closeup.get_width() - crop.x) / 2, (closeup.get_height() - crop.y) / 2)
	closeup.get_region(Rect2i(origin, crop)).save_png("%s/proc_field_closeup_crop.png" % OUTPUT_DIR)
	print("proc_tree_field_probe: saved proc_field_closeup.png at %s" % str(home))

	print("proc_tree_field_probe: DONE")
	scene.queue_free()
	await process_frame
	quit(0)
...
func _save_bake_check() -> void:
	var cols: int = 5
	var rows: int = int(ceil(float(_tex.size()) / float(cols)))
	var cell := Vector2i(360, 400)
	var sheet := Image.create(cols * cell.x, rows * cell.y, false, Image.FORMAT_RGBA8)
	sheet.fill(Color(0.30, 0.30, 0.33))
	for i: int in range(_tex.size()):
		var im: Image = _tex[i].get_image()
		if im.get_format() != Image.FORMAT_RGBA8:
			im = im.duplicate()
			im.convert(Image.FORMAT_RGBA8)
		var max_w: float = float(cell.x - 40)
		var max_h: float = float(cell.y - 40)
		var sc: float = minf(minf(max_w / float(im.get_width()), max_h / float(im.get_height())), 1.0)
		var nw: int = int(float(im.get_width()) * sc)
		var nh: int = int(float(im.get_height()) * sc)
		var im2: Image = im.duplicate()
		im2.resize(nw, nh, Image.INTERPOLATE_BILINEAR)
		var cx: int = (i % cols) * cell.x + (cell.x - nw) / 2
		var cy: int = (i / cols) * cell.y + (cell.y - nh) / 2
		sheet.blend_rect(im2, Rect2i(0, 0, nw, nh), Vector2i(cx, cy))
	sheet.save_png("%s/proc_bake_check.png" % OUTPUT_DIR)
	print("proc_tree_field_probe: saved proc_bake_check.png (инспекция текстур на обрезку)")
...
func _spawn_tree(world_pos: Vector2, variant: int, height_scale: float) -> void:
	...
	# Контактное пятно у основания (под травой).
	...
	# Тень-силуэт от солнца (под травой).
	...
	# Само дерево — на фиксированном слое над травой (z контейнера).
...
func _save_panel(frames: Array[Image], path: String) -> void:
	...
	var panel := Image.create(
		...
		frame_size.y, false, frames[0].get_format(),
	)
	panel.fill(Color.BLACK)
	for index: int in range(frames.size()):
		panel.blit_rect(frames[index], Rect2i(Vector2i.ZERO, frame_size), Vector2i(index * (frame_size.x + PANEL_DIVIDER_PX), 0))
	panel.save_png(path)
...
func _mean_abs_diff(a: Image, b: Image) -> float:
	...
	ai.resize(320, 180, Image.INTERPOLATE_BILINEAR)
	bi.resize(320, 180, Image.INTERPOLATE_BILINEAR)
	var total: float = 0.0
