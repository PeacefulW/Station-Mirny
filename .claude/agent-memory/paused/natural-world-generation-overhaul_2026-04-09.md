...
## 2026-04-09 — Reachable structure coverage Iteration 1 baseline

### Spec
- `docs/02_system_specs/world/reachable_structure_coverage_spec.md`

### Status
- Iteration 1 (`Coverage Proof Harness`) completed in `core/debug/world_preview_proof_driver.gd`.

### Proof
- Headless log: `debug_exports/world_previews/iteration1_structure_coverage_20260409_rerun.log`
- Summary artifact: `debug_exports/world_previews/structure_coverage_seed12345_1775735090.txt`
- River preview artifacts:
  - `debug_exports/world_previews/seed_12345_local_0_-4064_r24_1775735058_x4_biomes.png`
  - `debug_exports/world_previews/seed_12345_local_0_-4064_r24_1775735058_x4_terrain.png`
  - `debug_exports/world_previews/seed_12345_local_0_-4064_r24_1775735058_x4_structures.png`
- Mountain preview artifacts:
  - `debug_exports/world_previews/seed_12345_local_2464_-3648_r24_1775735090_x4_biomes.png`
  - `debug_exports/world_previews/seed_12345_local_2464_-3648_r24_1775735090_x4_terrain.png`
  - `debug_exports/world_previews/seed_12345_local_2464_-3648_r24_1775735090_x4_structures.png`

### Findings
- The sanctioned proof mode now prints a width-8 latitudinal band table, nearest authoritative/visible river and mountain distances from spawn, sparse native/script terrain parity, and writes a separate summary artifact under `debug_exports/world_previews/`.
- Fixed seed `12345` baseline:
  - `authoritative_river_cells=208`
  - `authoritative_mountain_cells=6489`
  - `visible_river_tiles=2`
  - `visible_mountain_tiles=1039`
  - `terrain_mismatches=0`
  - `structure_visibility_status=PASS`
- The new baseline confirms the world-scale regression shape instead of only hotspot anecdotes:
  - visible river samples remain effectively absent outside the polar river hotspot (`nearest_visible_river=4016.3 tiles`, `visible_water=none`, only `2` bank samples);
  - visible rock is reachable near spawn-facing central bands (`nearest_visible_rock=240.5 tiles`);
  - central latitude bands still show non-zero mountain visibility but zero sampled visible river hits.
- The harness also surfaced a likely Iteration 2 root-cause signal:
  - `sampled_authoritative_river_tiles=8191` across `8191` sparse samples, while exact pre-pass river cells are only `208`.
  - This strongly suggests the current `river_strength` handoff marks almost every sampled tile as an authoritative river-context consumer hit, even where the coarse pre-pass has no real river cell.

### Next step
- Use this baseline as the regression oracle for Iteration 2 river rebalance.
- Treat the “authoritative river everywhere” signal as a concrete follow-up target when reworking `WorldComputeContext._derive_river_strength_from_prepass()` and the native mirror.

## 2026-04-09 — Reachable structure coverage Iteration 2 river rebalance

### Spec
- `docs/02_system_specs/world/reachable_structure_coverage_spec.md`

### Status
- Iteration 2 (`River Coverage Rebalance`) completed across pre-pass hydrology, structure-context river semantics, mirrored native terrain consumption, and canonical docs.

### Proof
- Final structure-coverage log: `debug_exports/world_previews/iteration2_structure_coverage_20260409_final.log`
- Final structure summary artifact: `debug_exports/world_previews/structure_coverage_seed12345_1775737938.txt`
- Final native truth log: `debug_exports/world_previews/iteration2_native_truth_20260409_rerun2.log`
- Intermediate build/proof artifacts kept under `debug_exports/world_previews/iteration2_*` for audit trail.

### Findings
- `river_strength` no longer marks dry tiles as river-context by default:
  - Iteration 1 baseline had `sampled_authoritative_river_tiles=8191`;
  - final Iteration 2 summary reports `sampled_authoritative_river_tiles=104`.
- Central-band river visibility is now measurable on the fixed proof seed:
  - final summary shows `central_50` band `1024..2047` with `visible_river_samples=1`, `script_sand=1`, `native_sand=1`;
  - `nearest_visible_river` improved from `4016.3 tiles` in Iteration 1 to `2095.1 tiles`.
- Native/script parity remains intact after the rebalance:
  - final native truth proof reports `terrain_mismatches=0`, `variation_mismatches=0`, `flora_modulation_mismatches=0`, `native_truth_status=PASS`.
- The consumer-side widening was needed in addition to hydrology changes:
  - lowering river extraction threshold alone moved authoritative rivers closer to spawn, but sparse visible bank samples did not appear in central bands until the script/native terrain resolvers stopped over-trusting coarse radius gates over semantic `floodplain_strength` / `river_strength`.

### Notes
- A short-lived parity regression on `flora_modulation` was resolved by clearing hidden local-variation modulation when polar/salt overlays replace the visible variation id, so the payload no longer leaks divergent pre-overlay modulation values behind matching overlay ids.

## 2026-04-09 — Reachable structure coverage Iteration 3 mountain rebalance

### Spec
- `docs/02_system_specs/world/reachable_structure_coverage_spec.md`

### Status
- Iteration 3 (`Mountain Coverage Rebalance`) completed across pre-pass spine distribution, mountain-mass derivation, mirrored script/native terrain consumption, and canonical docs.

### Proof
- Native build log: `debug_exports/world_previews/iteration3_scons_build_20260409.log`
- Final structure-coverage log: `debug_exports/world_previews/iteration3_structure_coverage_20260409_rerun.log`
- Final structure summary artifact: `debug_exports/world_previews/structure_coverage_seed12345_1775738961.txt`
- Final native truth log: `debug_exports/world_previews/iteration3_native_truth_20260409.log`
- Failed first proof attempt kept for audit trail: `debug_exports/world_previews/iteration3_structure_coverage_20260409.log`

### Findings
- Spine seed placement no longer over-favors a few extreme-latitude maxima:
  - pre-pass selection now reserves the strongest still-valid candidate from each latitude band before unrestricted global fill, while still respecting configured Poisson spacing.
- `mountain_mass` now behaves as broader massif-fill support rather than a narrow ridge-core echo:
  - local ridge neighborhoods can sustain non-zero mountain support even when the rasterized ridge core softens between coarse cells.
- Fixed seed `12345` shows the intended reachability improvement near spawn:
  - `nearest_visible_rock` improved from `240.5 tiles` in the Iteration 1 baseline to `213.5 tiles`;
  - central latitude bands (`1024..2047`) remain strongly populated with authoritative mountains and visible mountain samples instead of collapsing into the extreme bands.
- Native/script parity remains intact after the mountain rebalance:
  - final native truth proof reports `terrain_mismatches=0`, `variation_mismatches=0`, `flora_modulation_mismatches=0`, `native_truth_status=PASS`.

### Notes
- The first Iteration 3 structure proof run failed fast on a GDScript parse error (`maxf()` called with too many arguments) in the new local-ridge helper; the rerun passed after narrowing that call to nested binary `maxf(...)`.
