...
## Главная сцена мира. Управляет спавном врагов,
## пикапов скрапа и проверкой indoor/outdoor для игрока.
...
func _ready() -> void:
	...
	# Начальные пикапы скрапа вокруг игрока
...
func _spawn_enemy() -> void:
	if not enemy_balance:
		return
	# Случайная позиция на расстоянии от игрока
	...
	# Создаём врага программно (в прототипе без .tscn)
...
func _spawn_initial_scrap() -> void:
	if not _player:
		return
	# 10 кусков скрапа вокруг точки старта
...
func _spawn_scrap_pickup(pos: Vector2) -> void:
	...
	pickup.collision_mask = 1  # Реагирует на игрока
	pickup.monitoring = true
	pickup.monitorable = false
	# Визуал — маленький жёлтый квадрат
	var visual := ColorRect.new()
