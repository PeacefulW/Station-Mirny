...
const WorldSpikyFloraScatterLayer = preload("res://core/systems/world/world_spiky_flora_scatter_layer.gd")

func _init() -> void:
	var layer := WorldSpikyFloraScatterLayer.new()
	...
	if plant_count <= 0:
		_fail("Spiky flora scatter produced no plants inside checked orange biofield masks.")
	if plant_count > WorldSpikyFloraScatterLayer.MAX_PLANTS_PER_CHUNK:
		_fail("Spiky flora scatter exceeded the per-chunk cap.")
	...
	if layer.get_plant_count() != 0:
		_fail("Spiky flora scatter must not place plants on an empty terrain mask.")

	print(
		"WORLD_SPIKY_FLORA_SCATTER_SMOKE chunks=%d plant_chunk=%s plants=%d" %
