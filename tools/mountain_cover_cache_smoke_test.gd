...
func _assert_roof_tileset_mapping() -> void:
	...
	_assert(int(render_debug.get("pending_mountain_id", 0)) == 1, "cover debug should keep mountain id after contour visual cutover")
	_assert(int(render_debug.get("pending_flags", 0)) == WorldRuntimeConstants.MOUNTAIN_FLAG_FOOT, "cover debug should keep mountain foot flags after contour visual cutover")
	_assert(int(render_debug.get("roof_cell_source_id", 0)) < 0, "contour visual cutover should not render mountain foot as a square roof TileMap cell")
	_assert((render_debug.get("roof_cell_atlas_coords", Vector2i.ZERO) as Vector2i) == Vector2i(-1, -1), "contour visual cutover should clear mountain foot roof atlas coords")
	view.free()
