"""Drive the full variation run: Blender bake -> canonical postprocess -> proof sheets.

Usage (from repo root):

    python tools/tree_atlas/make_tree_variations.py \
        --glb "M:/Downloads/1.glb" \
        --out-dir artifacts/layered_tree_v1_variations

Every variant goes through the canonical layered bake contract. This driver only
sequences the runs and renders the human-facing comparison sheets, one without
snow and one with the canonical snow overlay.
"""
...
def run_blender_variant(blender: Path, glb: Path, out_dir: Path, variant_id: str, variation_profile: Path) -> None:
    command = [
        str(blender),
        "-b",
        "--factory-startup",
        "--python",
        str(TOOL_DIR / "blender_tree_variation_bake.py"),
        "--",
        "--glb",
        str(glb),
        "--out-dir",
    ...
    expected = [out_dir / name for name in ("albedo.png", "trunk.png", "foliage.png", "shadow_raw.png", "classification.json")]
...
def variant_layers(asset_dir: Path, *, with_snow: bool) -> list[Image.Image]:
    layers = [
        ...
        POST.load_rgba(asset_dir / "trunk.png"),
        POST.load_rgba(asset_dir / "foliage.png"),
    ]
    if with_snow:
...
def build_world_scale_sheet(out_dir: Path, variants: list[dict], *, tile: int) -> Path:
    """One shared world scale for every variant, roots aligned on one baseline.

    The canonical camera refits each asset to the same frame fraction, so the
    per-asset sheets cannot show that the variants differ in size. This sheet
    undoes that refit using the recorded pixels-per-world-unit.
    """
    ...
    placed: dict[str, tuple[Image.Image, Image.Image, tuple[int, int]]] = {}
    ...
    draw.text(
        (14, 14),
        "Вариации дерева 1.glb — единый мировой масштаб (common world scale), корни на одной линии",
...
def build_sheet(
    ...
    *,
    ...
    label_font = load_font(21)
    sub_font = load_font(16)
    draw.text((14, 14), title, fill=(238, 230, 214, 255), font=title_font)

    for index, variant in enumerate(variants):
        asset_dir = out_dir / variant["id"]
        image = composite_variant(asset_dir, with_snow=with_snow)
        ...
        draw.text(
            (x + 12, title_h + 26),
            f"yaw {variant['yaw_degrees']:.0f}°  twist {variant['twist_degrees']:+.0f}°  h×{variant['height_scale']:.2f} w×{variant['width_scale']:.2f}",
...
def main() -> None:
    ...
    for variant in variants:
        ...
        if not args.skip_bake:
            ...
            run_blender_variant(args.blender, args.glb, asset_dir, variant["id"], args.variation_profile)
        POST.save_outputs(asset_dir, None, POST.load_profile(TOOL_DIR / "layered_asset_bake_profile.json"))
    ...
    no_snow = build_sheet(out_dir, ready, with_snow=False, tile=560, title="Вариации дерева 1.glb — без снега (no snow), покадровая подгонка камеры")
    snow = build_sheet(out_dir, ready, with_snow=True, tile=560, title="Вариации дерева 1.glb — со снегом (snow overlay), покадровая подгонка камеры")
