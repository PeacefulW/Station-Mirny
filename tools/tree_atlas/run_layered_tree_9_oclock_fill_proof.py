"""Run the isolated west 9 o'clock Sun plus calibrated 8% kicker proof."""
...
PRIOR = ROOT / "artifacts" / "layered_tree_brightness_variants" / "low_rear_fill_10_oclock"
...
REVIEW = Path(__file__).with_name("build_layered_tree_9_oclock_fill_review.py")
...
def main() -> None:
    ...
    profile["profile_id"] = "station_mirny_tree_sun_9_oclock_fill_08_proof"
    profile["version"] = 4
    profile["lighting"].update({
        "sun_azimuth_degrees": 270.0,
        "screen_sun_direction": "west_9_oclock",
        "fixed_shadow_direction": "screen_east",
        "fixed_shadow_direction_vector_screen": [1.0, 0.0],
        "ambient_fill": {"enabled": False},
        "fill_energy": 0.0,
    })
    profile["runtime"]["sun_shadow_mode"] = "baked_fixed_east_stretch_only"
    ...
    run([str(args.blender), "--background", "--factory-startup", "--python", str(DIAGNOSTIC), "--", "--glb", str(SOURCE_GLB), "--profile", str(profile_path), "--out-dir", str(diagnostic_dir)])
    if not (diagnostic_dir / "manifest.json").is_file():
        raise RuntimeError("9 o'clock diagnostic did not produce manifest")
    run([sys.executable, "-B", str(SELF_REPORT), str(diagnostic_dir / "self_shadow")])
    run([sys.executable, "-B", str(REVIEW), "--proof-dir", str(OUT), "--prior-dir", str(PRIOR)])
    ...
    if not guard["passed"]:
        raise RuntimeError("Production changed during isolated 9 o'clock proof")
    print(f"Wrote isolated 9 o'clock proof to {OUT}")
