"""Run isolated screen 10 o'clock Sun plus calibrated 20% kicker proof."""
...
REVIEW = Path(__file__).with_name("build_layered_tree_10_oclock_fill_20_review.py")
...
def main() -> None:
    ...
    run([
        ...
        "--target-lift", "20", "--spot-x", "2.25", "--spot-y", "-2.05",
    ...
    if not (diagnostic_dir / "manifest.json").is_file():
        raise RuntimeError("10 o'clock / 20% diagnostic did not produce manifest")
    run([sys.executable, "-B", str(SELF_REPORT), str(diagnostic_dir / "self_shadow")])
    run([
        sys.executable, "-B", str(REVIEW), "--proof-dir", str(OUT),
        "--twelve-dir", str(TWELVE), "--cast-asset-dir", str(CAST_ASSET),
    ])
    ...
    if not guard["passed"]:
        raise RuntimeError("Production changed during isolated 10 o'clock / 20% proof")
    print(f"Wrote isolated 10 o'clock / 20% proof to {OUT}")
