...
## 1. Главное: смысл `texture_scale` инвертирован (это баг)
...
Без этого крутить слайдер бесполезно.
...
## 3. Нормали слабые/резкие (под динамическое освещение это критично)
...
`apply_height_shading` (`render.rs:1577-1588`) запекает «фейковое
освещение» в albedo (`0.90..1.04`). При динамическом свете это
**двойная освещённость**: свет в шейдере + уже запечённая тень. Сделать
флаг `bake_height_shading` (default `false` для динамики). Albedo должен
быть «плоским цветом», когда есть нормали.
...
1. Поправить `texture_scale` (инвертировать или развести с `footprint`).
...
4. Sobel 3×3 + лёгкий blur для нормалей, `normal_strength` параметром,
   `bake_height_shading` опциональным флагом.
