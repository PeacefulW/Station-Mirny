...
func _assert_single_tile_result(world_core: Object) -> void:
	...
	_assert((result.get("collision_loops", []) as Array).size() == 1, "Single tile must emit one collision loop.")
...
func _assert_two_by_two_blob_result(world_core: Object) -> void:
	...
	_assert((result.get("collision_loops", []) as Array).size() == 1, "Face-connected 2x2 blob must emit one collision loop.")
...
func _assert_diagonal_contact_stays_separate(world_core: Object) -> void:
	...
	_assert((result.get("collision_loops", []) as Array).size() == 2, "Diagonal-only contact must remain two collision loops.")
