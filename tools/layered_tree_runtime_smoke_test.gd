...
const LAYERED_TREE_DIR_5: String = "res://assets/sprites/flora/layered_trees/tree_05"
const LAYERED_TREE_DIR_6: String = "res://assets/sprites/flora/layered_trees/tree_06"
...
func _run() -> void:
	...
	layer.set_layered_tree_asset_dirs([LAYERED_TREE_DIR, LAYERED_TREE_DIR_2, LAYERED_TREE_DIR_3, LAYERED_TREE_DIR_4, LAYERED_TREE_DIR_5, LAYERED_TREE_DIR_6])
	...
	_assert(int(state.get("tree_count", 0)) == 6, "WorldObjectPacketLayer must consume the layered tree packet.")
	...
	_assert(int(state.get("layered_tree_count", 0)) == 6, "Layered runtime renderer must contain six tree instances.")
	_assert(int(state.get("layered_tree_shadow_count", 0)) == 6, "Layered runtime renderer must contain one baked sun shadow per tree.")
	...
	if layered != null:
		...
		_assert(int(layered_state.get("asset_count", 0)) == 6, "Layered tree renderer must load all generated tree assets.")
		_assert(int(layered_state.get("loaded_asset_count", 0)) == 6, "Layered tree renderer must keep all layered tree assets ready.")
		_assert(int(layered_state.get("unique_asset_dir_count", 0)) == 6, "Layered tree renderer must use object variants to select all tree assets.")
		_assert(int(layered_state.get("instance_count", 0)) == 6, "Layered tree renderer must expose six visual trees.")
		_assert(int(layered_state.get("shadow_instance_count", 0)) == 6, "Layered tree renderer must expose six shadow trees.")
		...
		var shadow_direction: Vector2 = layered_state.get("shadow_direction_vector_screen", Vector2.ZERO) as Vector2
		_assert(shadow_direction.distance_to(Vector2(0.866025, 0.5)) < 0.001, "Layered sun shadow must use the selected 10 o'clock east-south-east direction.")
		_assert(absf(float(layered_state.get("shadow_contact_lock_source_px", 0.0)) - 48.0) < 0.01, "Layered sun shadow must preserve the authored 48 px contact lock.")
		var shadow_probe: Dictionary = layered_state.get("shadow_probe_local_points", {}) as Dictionary
		var frame_scale: float = float(layered_state.get("fixed_frame_scale", 0.0))
		for distance: int in [0, 16, 32, 48]:
			var expected: Vector2 = shadow_direction * float(distance) * frame_scale
			var actual: Vector2 = shadow_probe.get(str(distance), Vector2.INF) as Vector2
			_assert(actual.distance_to(expected) < 0.001, "Layered sun shadow contact probes must stay pinned while length changes.")
		var far_probe: Vector2 = shadow_probe.get("96", Vector2.ZERO) as Vector2
		_assert(far_probe.length() > 96.0 * frame_scale, "Layered sun shadow must stretch only beyond the contact lock.")
		...
		_assert(not _shader_contains("res://assets/shaders/layered_tree_foliage_wind.gdshader", "leaf_drop"), "Layered foliage must keep its full alpha silhouette in winter.")
		_assert(_shader_contains("res://assets/shaders/layered_tree_snow_accumulation.gdshader", "season_mask_texture"), "Layered snow must add foliage frost from the season mask.")
...
func _single_tree_packet() -> Dictionary:
	return {
		"object_kind": PackedByteArray([OBJECT_KIND_TREE, OBJECT_KIND_TREE, OBJECT_KIND_TREE, OBJECT_KIND_TREE, OBJECT_KIND_TREE, OBJECT_KIND_TREE]),
		"object_local_x_px_q4": PackedByteArray([80, 136, 192, 220, 64, 104]),
		"object_local_y_px_q4": PackedByteArray([128, 148, 132, 164, 176, 112]),
		"object_size_px": PackedByteArray([112, 180, 240, 208, 156, 196]),
		"object_atlas_index": PackedByteArray([0, 0, 0, 0, 0, 0]),
		"object_variant": PackedByteArray([0, 1, 2, 3, 4, 5]),
		"object_flags": PackedByteArray([1, 1, 1, 1, 1, 1]),
		"object_tint": PackedByteArray([64, 128, 255, 192, 96, 160]),
		"object_phase": PackedByteArray([0, 96, 192, 224, 32, 144]),
	}
...
func _shader_contains(path: String, needle: String) -> bool:
	if not FileAccess.file_exists(path):
		return false
