...
func _run() -> void:
	...
	layer.set_layered_tree_asset_dir(LAYERED_TREE_DIR)
	...
	_assert(int(state.get("tree_count", 0)) == 2, "WorldObjectPacketLayer must consume the layered tree packet.")
	...
	_assert(int(state.get("layered_tree_count", 0)) == 2, "Layered runtime renderer must contain two tree instances.")
	_assert(int(state.get("layered_tree_shadow_count", 0)) == 2, "Layered runtime renderer must contain one baked sun shadow per tree.")
	...
	if layered != null:
		...
		_assert(bool(layered_state.get("ready", false)), "Layered tree renderer must load tree_01 metadata.")
		_assert(int(layered_state.get("instance_count", 0)) == 2, "Layered tree renderer must expose two visual trees.")
		_assert(int(layered_state.get("shadow_instance_count", 0)) == 2, "Layered tree renderer must expose two shadow trees.")
...
func _single_tree_packet() -> Dictionary:
	return {
		"object_kind": PackedByteArray([OBJECT_KIND_TREE, OBJECT_KIND_TREE]),
		"object_local_x_px_q4": PackedByteArray([96, 172]),
		"object_local_y_px_q4": PackedByteArray([128, 148]),
		"object_size_px": PackedByteArray([112, 240]),
		"object_atlas_index": PackedByteArray([0, 0]),
		"object_variant": PackedByteArray([0, 0]),
		"object_flags": PackedByteArray([1, 1]),
		"object_tint": PackedByteArray([64, 255]),
		"object_phase": PackedByteArray([0, 192]),
	}
