...
func _run() -> void:
	...
	layer.set_layered_tree_asset_dirs([LAYERED_TREE_DIR, LAYERED_TREE_DIR_2, LAYERED_TREE_DIR_3])
	...
	_assert(int(state.get("tree_count", 0)) == 3, "WorldObjectPacketLayer must consume the layered tree packet.")
	...
	_assert(int(state.get("layered_tree_count", 0)) == 3, "Layered runtime renderer must contain three tree instances.")
	_assert(int(state.get("layered_tree_shadow_count", 0)) == 3, "Layered runtime renderer must contain one baked sun shadow per tree.")
	...
	if layered != null:
		...
		_assert(int(layered_state.get("asset_count", 0)) == 3, "Layered tree renderer must load all generated tree assets.")
		_assert(int(layered_state.get("loaded_asset_count", 0)) == 3, "Layered tree renderer must keep all layered tree assets ready.")
		_assert(int(layered_state.get("unique_asset_dir_count", 0)) == 3, "Layered tree renderer must use object variants to select all tree assets.")
		_assert(int(layered_state.get("instance_count", 0)) == 3, "Layered tree renderer must expose three visual trees.")
		_assert(int(layered_state.get("shadow_instance_count", 0)) == 3, "Layered tree renderer must expose three shadow trees.")
...
func _single_tree_packet() -> Dictionary:
	return {
		"object_kind": PackedByteArray([OBJECT_KIND_TREE, OBJECT_KIND_TREE, OBJECT_KIND_TREE]),
		"object_local_x_px_q4": PackedByteArray([80, 136, 192]),
		"object_local_y_px_q4": PackedByteArray([128, 148, 132]),
		"object_size_px": PackedByteArray([112, 180, 240]),
		"object_atlas_index": PackedByteArray([0, 0, 0]),
		"object_variant": PackedByteArray([0, 1, 2]),
		"object_flags": PackedByteArray([1, 1, 1]),
		"object_tint": PackedByteArray([64, 128, 255]),
		"object_phase": PackedByteArray([0, 96, 192]),
	}
