...
func _init() -> void:
	...
	_assert(
		_gdscript_composition_uses_image_operations(),
		"overview composition should stay on image operations and must not read hydrology snapshots in GDScript"
	)
...
func _gdscript_composition_uses_image_operations() -> bool:
	var backend_source: String = FileAccess.get_file_as_string("res://core/systems/world/world_chunk_packet_backend.gd")
	if not backend_source.contains("blend_rect"):
		return false
	if backend_source.contains("get_world_hydrology_snapshot"):
		return false
	if backend_source.contains("get_pixel(") or backend_source.contains("set_pixel("):
		return false
	return true
