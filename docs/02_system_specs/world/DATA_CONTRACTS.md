...
- `readers`: `WorldGenerator.initialize_world()` boot sequence, `WorldComputeContext.get_world_pre_pass()`, `WorldComputeContext.sample_prepass_channels()`, `WorldComputeContext.sample_structure_context()`, `WorldLab`, and future generator-side samplers/resolvers that explicitly opt into prepass channels. Current runtime world/chunk/mining/topology/reveal/presentation systems do not read this layer directly.
