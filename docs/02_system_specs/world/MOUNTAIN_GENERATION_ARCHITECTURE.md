...
- `RoofLayer` (derived): визуальный оверлей поверх террейна. Рисуется
  **только там, где плитка имеет `mountain_id > 0` и `is_interior_flag == 1`**,
...
# становится: _base_layer, _overlay_layer, _roof_layer

_roof_layer = TileMapLayer.new()
_roof_layer.name = "MountainRoofLayer"
_roof_layer.tile_set = WorldTileSetFactory.get_roof_tile_set()
_roof_layer.z_index = 10  # поверх overlay
_roof_layer.texture_filter = CanvasItem.TEXTURE_FILTER_NEAREST
```

**Атлас крыши = rock-top атлас.** Буквально те же спрайты, что для
`TERRAIN_MOUNTAIN_WALL`. Чтобы крыша снаружи выглядела как продолжение горы
без шва, autotile-47 индекс на крыше для клетки `(wx, wy)` считается так же,
как для rock-wall: по соседям, у которых `mountain_id == текущий`.
...
соседствующий хотя бы с одним **не-mountain** тайлом (`mountain_id == 0`),
...
**Решение принято** (см. §10, Q1): **по одному roof-`TileMapLayer` на
каждый `mountain_id`** в чанке. Без агрегации. Агрегированный alpha на чанк
**запрещён**, потому что нарушает ключевой UX-инвариант «opening mountain A
must not reveal mountain B» на стыках гор.

```
ChunkView layout:
  [z=0]   _base_layer                          — ground (plains), rock-wall mountain faces
  [z=1]   _overlay_layer                       — существующий (deco, etc.)
  [z=10+] roof_layers_by_mountain[mountain_id] — по одному слою на каждую гору в чанке
```

Внутри `ChunkView`:

```gdscript
var roof_layers_by_mountain: Dictionary = {}  # int mountain_id -> TileMapLayer

func _ensure_roof_layer(mountain_id: int) -> TileMapLayer:
    if roof_layers_by_mountain.has(mountain_id):
        return roof_layers_by_mountain[mountain_id]
...
func _on_mountain_reveal_alpha_changed(mountain_id: int, alpha: float) -> void:
    var layer: TileMapLayer = roof_layers_by_mountain.get(mountain_id, null)
    if layer == null:
        return
...
> not per chunk. Each `ChunkView` keeps `Dictionary[int, TileMapLayer]
> roof_layers_by_mountain`. Reveal alpha is applied per mountain layer, so
> adjacent mountains in the same chunk can open/close independently.
...
- `core/systems/world/chunk_view.gd` — добавить `_roof_layer`,
  `set_roof_alpha`, reveal-subscription
- `core/systems/world/world_tile_set_factory.gd` — `get_roof_tile_set()`
