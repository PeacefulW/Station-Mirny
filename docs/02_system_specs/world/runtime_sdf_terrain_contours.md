...
- All active ground and mountain contour classes must be generated through the
  native contour field path. GDScript constant contour results and visual
  placeholders are not valid cutover behavior.
...
If a loaded chunk contains any of those terrain ids and no matching native
contour result has been published for the required revision, the chunk remains
hidden and movement into the missing contour region remains not ready. If a
later dirty revision is pending, the previous visual revision may remain
visible, but movement into the stale contour region still returns not ready.
This is an intentional fail-closed readiness rule, not a fallback.
...
| `ground_surface` | walkable ground and dug terrain | authored ground contour material under active cutover terrain; generated by the same native SDF contour path as mountains | non-blocking |
...
Ground is not a blocking solid, but it is still a visual contour class.
`ground_surface` must use the authored recipe and native SDF field so ground
edges, dug cuts, and mountain contact edges share the generator's contour
language. Organic blocking shape, visible mountain occupancy, and movement
...
the color source. Dynamic lighting belongs to the game lighting layer and normal
maps, not to baked albedo.

Rules:

- exported `top_albedo`, `face_albedo`, and `base_albedo` are material color
  truth for contour rendering
- exported `top_normal`, `face_normal`, and native SDF normals are lighting
  response truth
...
Immediate shader-side visual cutouts are not allowed during dirty refresh.
They create a third temporary art model between the authored export and the
native SDF contour result. A dirty chunk may keep the previous visual revision
visible while movement into the affected dirty region remains blocked. The next
visible change must be the full affected chunk worker refresh and matching
visual/collision revision swap.
