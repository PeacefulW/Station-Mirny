...
object_flags: PackedByteArray         # reserved; current emitter writes 0; tree collision derives from kind 4, small rock has none
...
`WorldCore` alongside terrain data. Worker-produced family buffers enter the
global `WorldRenderWorld` through data-driven RenderClassRegistry source
bindings; compact tree rectangles enter chunk-local
`WorldObjectCollisionOwner`. Neither consumer owns gameplay truth.
...
Production generated objects are rendered only by world-parented
`WorldRenderWorld`. `ChunkView` adopts a collision-only owner by reference and
does not own gameplay truth or static object pixels.
...
- trees use `object_kind == 4`; presentation resolves through the global
  layered-tree RenderClass descriptor, and trunk collision is chunk-scoped on
  one `StaticBody2D` per collision owner.
...
### Global Batch Presentation Contract

Production presentation for mass objects must use data buffers and
fixed-pass global `MultiMeshInstance2D` streams, not per-object `_draw()` calls.
Family/variant selection is registry data; painter order is global and native.
Chunk-local presentation retains only collision ownership.

> The long per-chunk batching/pool/stripe implementation record below is kept
> for migration history and is superseded in full by `object_render_world.md`
> and the Shared RenderWorld Painter amendment in `world_runtime.md`. It is not
> permission to restore a compatibility renderer.

Required direction:

- one chunk object placement result produces compact presentation buffers;
- `ChunkPacketV1.object_*` remains the immutable placement source; a worker-local
  `WorldCore.build_object_presentation_buffers(...)` call derives a separate
  transient `ObjectPresentationBufferResult`. The result is never inserted back
  into the packet and is never saved;
...
- layered tree and small-rock source channels are packed offline without
  resampling into boot-preloaded atlases. Runtime chunk publication must not
  parse asset JSON, call `load()`, or allocate a material per object/chunk;
- native compute groups instances by chunk-local depth stripe and writes the
  final 12-float `MultiMesh.buffer` layout. GDScript must not split records or
  call per-instance `MultiMesh` setters in the production path;
- living flora, its contact shadows, and both spiky-flora atlas banks are packed
  during that same single native packet pass. Their optional bucket tables stay
  empty when the family count is zero; enabling either family must not restore a
  synchronous packet rescan or main-thread depth split;
- the worker presentation policy follows actually prepared sources. Known
  living/spiky records may remain in canonical native placement while their
  current runtime sources are disabled; those records are counted as suppressed,
  produce no buffer/cache bytes, do not fail chunk publication, and do not alter
  current visuals. Once the matching sources are prepared, the same packet
  records enter the exact buffered path without a packet/schema migration;
- object preparation is queued by `epoch + chunk_coord + revision`, reprioritized
  with the current streaming bubble, and stale queued/completed work is rejected.
  Enqueue retains immutable packet/catalog `PackedArray` handles through
  copy-on-write ownership in O(1) relative to payload size; producer and worker
  only read the shared backing storage through completion.
  Reveal is the normal strict priority over hidden source-padding decode; bounded
  shared-pool fairness grants one oldest streaming request after five consecutive
  reveal request dispatches. Every native packet-batch member accrues debt, and a
  fairness grant cannot open a batch, preventing vehicle prefetch starvation
  without creating a low-priority burst ahead of the live frontier;
- a current-revision native or main-thread contract failure gets at most two
  delayed, distance-prioritized rebuild attempts. A catalog-generation mismatch
  releases the stale inflight owner and queues current-catalog work. Exhaustion
  enters the explicit compatibility renderer as a last-resort recovery for that
  revision: no tight retry loop and no permanently hidden chunk. The recovery
  remains behind the same atomic reveal/collision gate;
- main-thread apply is incremental by a bounded number of **non-empty**
  stripes/colliders; the fixed 64-entry bucket table may be scanned past empty
  entries in the same slice because those entries perform no scene/GPU work. A
  newly loaded chunk becomes visible only after tree visuals, living flora and
  shadows, both spiky-flora atlas banks, small-rock visuals, and the chunk-scoped
  trunk collision derived from the same result are committed together. Cosmetic
  object families must not visibly pop in after chunk reveal. Prepared shape
  owners keep `collision_layer = 0` while
  the owning `ChunkView` is hidden (including mountain-mask waits); the streamer
  activates them only in the actual chunk reveal transaction;
- a bounded RAM-only warm cache may retain prepared CPU buffers across temporary
  zoom/radius eviction. It must be invalidated on generation epoch/catalog
  change and must not retain scene nodes or GPU ownership. A separate bounded
  hot cache may retain only fully committed GPU layers, accounted independently
  by chunk count, raw buffer bytes, CanvasItems, and colliders. It never stores a
  partial layer as ready. `WorldStreamer` owns each world-parented staging
  envelope; `ChunkView` only adopts a completed layer by reference. Neither owner
  may mutate/clear the cache-owned result Dictionary or its immutable packed
  payloads. A layer cancel/recycle drops local packed-array references rather
  than clearing shared worker arrays;
- residency accounting covers the complete retained CanvasItem graph (family
  owners, depth-ladder owner and its three band roots, optional contact shadow,
  collision/debug owners, and draw leaves), not only visible draw leaves. Dense
  pooled buffers that are inactive after sparse reuse are reset before the
  committed exact weight is recorded, so a nominal byte cap cannot hide old
  `MultiMesh` capacity;
- an incomplete layer required by a live hidden `ChunkView` is transient reveal
  working set, not an evictable cache entry. Cache trimming may temporarily leave
  that bounded visible-ring working set above the reusable-residency cap; it must
  not evict and synchronously restage the same transaction. Once complete, the
  layer is promoted into its view or becomes an ordinary cache victim;
- main-thread object scheduling is completion-biased: live/reveal work first,
  one focused transaction to `COMPLETE`, hidden source-padding prestage only from
  leftover budget. Packed results wait in a lightweight queue; the streaming
  result-drain callback must not allocate a layer per completed worker result.
  The unique-token visual queue is hard-capped to the current plus outgoing
...
- eviction is queue-only. A separate retire dispatcher performs exactly one
  ...
  victim, or pool-shrink operation per callback and always yields. Active,
  retiring, and pooled graphs remain in conservative GPU/CanvasItem/collider
  accounting until the corresponding resource is actually released. Source-only
  admission pauses while retirement or a residency dimension is over budget,
  retains its token, and retries automatically; live/reveal work bypasses that
  backpressure. The pool accepts only fully clean layers and shrinks overflow one
  resource at a time;
- a promoted hot layer remains under the world presentation root and is revealed
  by its owning `ChunkView` without a Node/CanvasItem reparent or raw buffer
  reupload. Externally parented layers stay invisible and collision-free until
  the chunk reveal transaction activates both;
- layered `QuadMesh` visual shaders convert primitive V to PNG orientation once
  (`1.0 - UV.y`) before every channel/mask/wind sample. The custom shadow mesh is
  already Canvas-oriented and is not flipped;
- only the compatibility fallback may host a batch helper inside `ChunkView`; it
  must not compute canonical placement truth.
- `WorldRenderWorld` consumes native family buffers as one fixed-pass global
  snapshot. `WorldObjectCollisionOwner` consumes only tree rectangle records;
  no chunk-local object sprite/shadow MultiMesh exists in production.
...
- rebuilding immutable object buffers for a terrain-only diff or camera zoom.
...
- native collision output uses stride `4`:
  `(center_x, center_y, width, height)`. `WorldObjectCollisionOwner` realizes each
...
- object buffer decode/bucketing/packing: native background worker compute;
...
- at least `10x` the current authored tree/rock density without node count,
  material count, JSON parses, or texture loads scaling per object;
