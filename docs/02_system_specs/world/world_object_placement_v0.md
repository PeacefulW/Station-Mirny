...
V0 should extend the chunk packet additively with a compact placement section.

Direction:

```text
object_ids: PackedInt32Array          # registry-stable numeric indices or IDs
object_local_x: PackedByteArray       # 0..15 in the current 16x16 chunk
object_local_y: PackedByteArray       # 0..15
object_variant: PackedByteArray       # visual variant / atlas region
object_flags: PackedByteArray         # bit flags, reserved values documented
```

If string IDs are too expensive for the hot packet, the registry must own the
stable ID to numeric index mapping. The mapping must be deterministic for the
loaded content pack set and must not depend on random file order.
...
- `ChunkView` may host the batch helper, but it must not compute canonical
  placement truth.
- temporary local scatter adapters are allowed only while generated object
  content remains disabled; accepted dense placement must move into the native
  packet path before content is enabled at scale.
...
- add a minimal `plains`-only placement path for non-interactive generated
  objects.
- current implementation may keep generated object content disabled while the
