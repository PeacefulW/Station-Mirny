...
`WorldCore` alongside terrain data and consumed by `WorldObjectPacketLayer`.
Production layers are staged under `WorldStreamer`'s world-presentation root and
adopted by `ChunkView` by reference; only the compatibility fallback may remain
`ChunkView`-local.
...
Production generated objects are rendered by the world-parented
`WorldObjectPacketLayer`; `ChunkView` adopts the completed layer by reference and
does not own gameplay truth. A view-local helper is compatibility fallback only.
