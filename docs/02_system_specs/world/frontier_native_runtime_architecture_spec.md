...
- `R6` routes staircase traversal through `UndergroundTransitionCoordinator`: fade-out may start immediately, but runtime publication is hidden during blackout, `ZLevelManager.change_level()` is not allowed to reveal the target level on its own, and fade-in waits until the active player hot `3x3` envelope is terminal `full_ready`.
