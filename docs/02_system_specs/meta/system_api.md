...
| `build_chunk_halo_fields(packets_3x3: Array, halo_radius_tiles: int)` | `Dictionary` | Pure derived worker helper over exactly nine row-major shallow immutable packet envelopes; explicit `{ "halo_source_present": false }` slots represent finite north/south void boundaries. Returns packed `remaining_halo`, `closed_halo`, `dug_halo`, and `terrain_edge_solid_halo` fields plus O(1) counts. Grass requests may consume `remaining_halo` on the same worker before native scatter packing; no scene/GPU/save/gameplay state or main-thread packed-channel scan. |
| `build_grass_scatter_buffer(seed: int, chunk_coord: Vector2i, terrain_ids: PackedInt32Array, lake_flags: PackedByteArray, mountain_halo: PackedByteArray, mountain_halo_radius_tiles: int, params: PackedFloat32Array)` | `Dictionary` | Presentation-only deterministic grass tuft placement for one chunk; returns the `GrassScatterBufferResult` shape from `packet_schemas.md` (ready `MultiMesh` buffer). `mountain_halo` is the exact cross-chunk `remaining_halo` derived from the fixed 3x3 packet window; it may arrive explicitly or be derived by the same worker immediately before scatter packing. Density mirrors the ground material's aperiodic world fields; never packet truth, save state, collision, or walkability. |
| `build_grass_render_page_buffer(contributors: Array, page_width_chunks: int, chunk_size_px: float)` | `Dictionary` | Pure presentation worker helper for the full-LOD `4 x 1` grass render-page path. Merges up to four immutable `GrassScatterBufferResult` contributor handles into `GrassRenderPageBufferResult`, preserving every packed float except the exact slot-local X rebase. Called only on worker-local `WorldCore`; no scene/GPU/save/gameplay state and no GDScript instance traversal. |
