...
| `build_mountain_halo_mask(solid_halo: PackedByteArray, chunk_size: int, tile_size_px: int, pixels_per_tile: int, origin_world_x: float, origin_world_y: float, cutout_halo: PackedByteArray = PackedByteArray())` | `Dictionary` | Derived native mask helper for runtime mountain and terrain-edge presentation. For mountains, optional effective excavation ownership produces both open and closed-roof masks. Returns `MountainHaloMaskResult` from `packet_schemas.md`; not packet truth, save state, collision, or terrain ownership. |
| `compose_mountain_cover_mask(open_mask: PackedByteArray, roof_mask: PackedByteArray, visibility_halo: PackedByteArray, pixels_per_tile: int)` | `PackedByteArray` | Presentation-only compositor selecting organic excavation cutouts for real mouths/current cavity while retaining the same native closed roof elsewhere. It is called from the budgeted visual-upload path and never mutates packet, diff, terrain, walkability, or save state. |
