...
Parsed by native `WorldCore.build_contour_chunk(input)` from the exported
runtime SDF contour recipe JSON.
...
Current code notes:
- the recipe is input configuration for a derived runtime contour cache, not
...
Input dictionary accepted by native `WorldCore.build_contour_chunk(input)`.
...
Current code notes:
- `solid_mask_with_halo` is compact tile data with side
  `chunk_size_tiles + halo_tiles * 2`
- `tile_size_px` is the logical contour geometry scale. `render_tile_size_px`
  is an optional visual-cache resolution; when omitted, native compute uses
  `tile_size_px`. Collision output remains in logical `tile_size_px`
  coordinates.
- positive mask values mean inside/solid; native signed distance is positive
  inside and negative outside
- X wrapping and Y empty/out-of-world treatment are input-assembly
  responsibilities; this native boundary consumes only the compact mask it is
  given
- the input mask represents already resolved `base + diff` state and is not
  itself persisted by this contour API
- Runtime SDF Contours Iteration 03 assembles this input in `WorldStreamer`
  from loaded packets, `WorldDiffStore`, and transient generated base packets
  for missing halo neighbors. Those generated packets are worker-local input
  data only and do not become `ChunkPacketV1` readiness.
- GDScript debug readback may expose extra halo diagnostics such as
  `source_mask_with_halo`, `terrain_id_with_halo`, and
  `chunk_coord_with_halo`; these are not accepted native schema fields and are
  not persisted.

### `ContourChunkResultV1`

Returned by native `WorldCore.build_contour_chunk(input)`.
...
```text
{
  ...
  "collision_origin_world_px": Vector2i,
...
Current code notes:
- for the current runtime cache, one `16 x 16` chunk at logical `64 px` and
  `render_tile_size_px = 32` emits `pixel_size = Vector2i(512, 512)`;
  fixture/native parity calls may omit `render_tile_size_px` and emit
  `Vector2i(1024, 1024)`
- `mask_rgba8` is `pixel_size.x * pixel_size.y * 4` bytes with channel semantics:
  R = top coverage, G = face coverage, B = back coverage, A = occupancy
- `height_r16` is `pixel_size.x * pixel_size.y * 2` little-endian IEEE 754 binary16 bytes
  storing normalized `0..1` surface height for Godot `Image.FORMAT_RH`
- `normal_rgba8` is `pixel_size.x * pixel_size.y * 4` bytes and uses the same alpha occupancy
  as `mask_rgba8`
...
- `collision_sdf_f32` is sampled from the same signed distance field at
  `collision_sample_px`; with `sampling_px = 4`, `collision_size` is
  `Vector2i(257, 257)`
- this result is derived cache data. It is not part of `ChunkPacketV1`, not
  save data, and must be regenerated from authoritative base/diff masks when
  needed
