...
- Когда вызывать: только из chunk lifecycle owner path, если уже созданный loaded chunk нужно redraw immediately. In boot path, called for ring 0 (player chunk) with `include_flora=true`. In runtime streaming, called for near-player chunks (Chebyshev ring 0-1) at finalize to eliminate green placeholder zones.
...
- Гарантии: payload fields соответствуют `ChunkBuildResult.to_native_data()` contract из `DATA_CONTRACTS.md`; sync и worker/native build paths обязаны выдавать один и тот же `feature_and_poi_payload`. When `use_native_chunk_generation` enabled, terrain/height/variation/biome/flora arrays generated by C++ `ChunkGenerator.generate_chunk()` (×5-10 faster); `feature_and_poi_payload` always computed in GDScript. Falls back to GDScript if native DLL unavailable. Any debug presentation proof must stay downstream of this built payload instead of recomputing feature / POI decisions.
