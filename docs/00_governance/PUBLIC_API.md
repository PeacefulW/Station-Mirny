...
- Что делает: loads chunks with per-chunk readiness tracking and honest post-handoff continuation owned internally by `ChunkBootPipeline`. Startup visual work is scheduled through the budgeted chunk visual scheduler and chunk-local `ChunkVisualState`; fresh chunks stay hidden until terminal `Chunk.is_full_redraw_ready()` publication closes. If detached/native critical compute is unavailable, boot/runtime now emit a zero-tolerance breach and fail closed instead of taking sync/compatibility fallback. Unfinished startup coords after internal `first_playable` remain boot-tracked and continue through runtime scheduling until real completion.
