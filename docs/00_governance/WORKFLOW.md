...
2. Feature spec текущей задачи (если есть)
3. Только после этого — конкретные файлы, указанные в задаче или контракте
...
Если задача не указывает конкретный файл — посмотри в `DATA_CONTRACTS.md`, там указаны конкретные файлы и функции для каждого слоя.
...
2. Прочитай spec текущей фичи (если есть) — пойми acceptance tests для своей итерации.
3. Определи, какие слои данных затрагивает твоя задача.
4. Определи, какие конкретные файлы и функции указаны в задаче и контракте. Работай ТОЛЬКО с ними.
...
3. Найди в контракте: какой файл и функция отвечают за этот инвариант?
4. Открой ТОЛЬКО этот файл. Исправь ТОЛЬКО эту функцию.
