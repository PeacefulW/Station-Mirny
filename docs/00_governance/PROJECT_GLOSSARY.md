...
### Riverbed
Canonical dry channel terrain generated by worldgen from the river substrate.
It remains visible when water disappears and is not the same thing as water.
Riverbed tiles are immutable base data unless a future player mutation command
explicitly creates a runtime diff.

### Riverbed depth
Tile-level channel depth class for river and lake beds. Current intended
classes are `none`, `shallow`, and `deep`. `deep` means the bed depression, not
an automatic movement blocker; future deep water blocks movement through the
water overlay.

### Lakebed scar
Dry footprint of a lake candidate. It is visible before water exists and during
drought, and it is regenerated from seed/version/settings rather than saved per
tile.

### Ocean-directed trunk
A visible river trunk whose downstream chain reaches the top-Y ocean band.
River Generation V1 realizes only ocean-directed primary riverbeds as
player-facing river channels.

### Side channel
A secondary deterministic river branch that splits from a primary trunk and
rejoins downstream or terminates in a valid lake/ocean outlet. Side channels
create islands but do not replace the authoritative `WorldPrePass` downstream
graph.

### Water overlay
Future runtime/environment layer that says how much water is present on top of
riverbeds, lakebeds, or other water-capable terrain right now. It may affect
walkability and presentation, but it must not rewrite immutable base terrain.
