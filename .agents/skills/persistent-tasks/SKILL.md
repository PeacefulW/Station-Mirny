...
description: >
  ...
  context loss when a long feature implementation spans multiple agent sessions.
  ...
  that has multiple iterations. Also use when `mirny-task-router` classifies the request
  as multi-iteration or resume-sensitive work, or when you notice the current task has
  more than 3 steps and might not fit in one session.
---
